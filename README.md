...
Comprehensive guides are available in the `docs/` directory to help you get the most out of this template:

### Core Guides

- **[🚀 DEPLOYMENT.md](docs/DEPLOYMENT.md)** - Production deployment configuration
  - Environment variable flags (`USE_SUPERVISOR`, `RUN_CELERY_TOGETHER`)
  - Deployment scenarios (Standard, All-in-One, Orchestrated)
  - Complete file reference for the `deploy/` directory

- **[🛠️ SETUP.md](docs/SETUP.md)** - Manual setup without Makefile
...
- **[📦 PACKAGE_MANAGER.md](docs/PACKAGE_MANAGER.md)** - Dependency management with `uv`
  - Complete `uv` tutorial and best practices
  - Adding, removing, and updating packages
  - Understanding `pyproject.toml` and `uv.lock`

### API Documentation

- **[📖 Swagger UI](docs/swagger.html)** - Interactive API documentation (generate with `make generate-docs`)
- **[📖 ReDoc](docs/redoc.html)** - Alternative API documentation viewer
...
# 2. Initialize project (creates venv, installs deps)
make init
...
If you prefer not to use `make`, see the **[Manual Setup Guide](docs/setup.md)**.
...
| Command | Description |
|---------|-------------|
| `make install` | Sync dependencies from lock file |
| `make add-package PACKAGE=name` | Add a new package |
