...
def ping_celery_task(self, duration: int = 0, trace_id: str = None) -> dict:
    ...
    """
    if trace_id:
        logger.info(f"Task started | trace_id={trace_id} | service=celery | duration={duration}")
    ...
    try:
        ...
        if trace_id:
            logger.info(f"Task success | trace_id={trace_id} | service=celery")
            cache.set(f"task_status:{trace_id}", {"status": "SUCCESS", "result": result}, timeout=300)

        return result
    except Exception as e:
        if trace_id:
            logger.error(f"Task failed | trace_id={trace_id} | service=celery | error={str(e)}")
